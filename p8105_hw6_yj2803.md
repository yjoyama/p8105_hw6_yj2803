...
```
    ...
    ##     smoken + ppbmi + babysex + parity + ppwt + fincome, data = df_birthwt)
    ## 
    ## Coefficients:
    ## (Intercept)        bhead      blength       mrace2       mrace3       mrace4  
    ##   -5683.316      130.839       74.978     -138.830      -76.349     -101.778  
    ##       delwt      gaweeks       smoken        ppbmi     babysex2       parity  
    ##       4.109       11.581       -4.844       -9.222       28.557       95.895  
    ##        ppwt      fincome  
    ##      -1.068        0.323

First, I fit the intercept-only model. Then, predictors were added to
the model sequentially. After adding each predictor, any predictors that
did not improve in model fit were removed from the model. This process
was repeated until obtaining the final model.

The predictors included in the final model are:  
`bhead`, `blength`, `mrace`, `delwt`, `gaweeks`, `smoken`, `ppbmi`,
...
![](p8105_hw6_yj2803_files/figure-gfm/unnamed-chunk-8-1.png)<!-- -->

We can see that the spread of the residuals are higher for the lower
fitted values, but overall (especially when the predicted values \>
2000) the points are scattered randomly around the residual = 0 line.
...
![](p8105_hw6_yj2803_files/figure-gfm/unnamed-chunk-9-1.png)<!-- -->

I split the dataset into two and generated 100 test-training pairs.
Then, each linear models were applied to these datasets and root mean
squared errors (RMSEs) were computed for each model.  
The plot shows the RMSE values for each model. It suggests that the
final model fits better than the other two models.
